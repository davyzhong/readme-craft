...
readme-craft 是一套面向 AI Agent 的 README 方法论、规则引擎与工具链。核心目标：基于真实项目证据，稳定产出结构清晰、可执行、无虚假信息的 README。
...
- `skill/`：可安装的 craft-readme Skill 源文件。
- `assets/`：视觉资产。
- `scripts/`：CLI 与工具脚本。
...
v3.0.0-alpha.0 已发布（npm + GitHub Action v1 + GitHub Pages 评分页）。审批门 1-3 已关闭。待决：v3.0 正式版 tag / Release 时机。
